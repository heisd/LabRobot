...
#include "robot_interfaces/srv/set_rgb.hpp"
...
{
	public:
		turn_on_robot(std::string);  //Constructor //构造函数
	...
	private:
		...
		//红外对接速度话题订阅回调函数
		void Red_Vel_Callback(const geometry_msgs::msg::Twist &twist_aux); 
		...
		rclcpp::Publisher<robot_interfaces::msg::Supersonic>::SharedPtr distance_publisher;         // CHANGE
		rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr Charging_publisher;         // CHANGE
        ...
        string usart_port_name, robot_frame_id, gyro_frame_id, odom_frame_id,robot_type,car_mode,cmd_vel; //Define the related variables //定义相关变量
