...
#include "turn_on_wheeltec_robot/Quaternion_Solution.hpp"
...
int main(int argc, char** argv) 
{
  ...
  turn_on_robot Robot("wheeltec_robot"); //Instantiate an object //实例化一个对象
  Robot.Control(); //Loop through data collection and publish the topic //循环执行数据采集和发布话题等操作
  rclcpp::shutdown();
  return 0;  
} 
/**************************************
...
***************************************/
void turn_on_robot::Red_Vel_Callback(const geometry_msgs::msg::Twist &twist_aux)
{
  short  transition;  //intermediate variable //中间变量
...
***************************************/
void turn_on_robot::Recharge_Flag_Callback(const std_msgs::msg::Int8 &Recharge_Flag)
{
...
void turn_on_robot::Publish_Voltage()
{
    ...
    time_period=(rclcpp::Node::now() - Last_Time).seconds();
...
void turn_on_robot::Control()
{
  ...
  while(rclcpp::ok())
  {
    ...
    current_time = rclcpp::Node::now();
    Sampling_Time = (current_time - last_time).seconds(); //Retrieves time interval, which is used to integrate velocity to obtain displacement (mileage) 
...
turn_on_robot::turn_on_robot(std::string node_name):Node(node_name),Sampling_Time(0),Power_voltage(0)
{
  ...
  this->declare_parameter<std::string>("car_mode", "S300");
  this->declare_parameter<std::string>("robot_type", "Plus");
  ...
  odom_publisher             =     create_publisher<nav_msgs::msg::Odometry>("odom", 10);    // CHANGE
  distance_publisher         =     create_publisher<robot_interfaces::msg::Supersonic>("Distance", 10);    // CHANGE
  Charging_publisher         =     create_publisher<std_msgs::msg::Bool>("robot_charging_flag", 20);    // CHANGE
  ...
  //速度控制命令订阅回调函数设置
  Cmd_Vel_Sub     = create_subscription<geometry_msgs::msg::Twist>(cmd_vel, 100, std::bind(&turn_on_robot::Cmd_Vel_Callback, this,std::placeholders::_1)); 
  //Ir docking speed topic subscription callback function
  //红外对接速度话题订阅回调函数
  Red_Vel_Sub       = create_subscription<geometry_msgs::msg::Twist>("red_vel", 100, std::bind(&turn_on_robot::Red_Vel_Callback, this,std::placeholders::_1)); 
  //Automatic recharge mode flag bit subscription callback function
  //自动回充模式标志位订阅回调函数
  Recharge_Flag_Sub = create_subscription<std_msgs::msg::Int8>("robot_recharge_flag",100, std::bind(&turn_on_robot::Recharge_Flag_Callback, this,std::placeholders::_1)); 

  //底盘安全防护关闭话题订阅
  Security_Sub = create_subscription<std_msgs::msg::Int8>("chassis_security",100, std::bind(&turn_on_robot::Security_Callback, this,std::placeholders::_1)); 
  try
  { 
