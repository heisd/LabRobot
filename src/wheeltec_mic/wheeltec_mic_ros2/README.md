...
通过串口 with 麦克风阵列通讯,实现:
- **唤醒检测**:发布唤醒标志、麦克风指向角度。
- **命令词识别**:把"前进/后退/左转/跟随我"等口令转换为 `MotionControl` 消息。
- **兼容性支持**:支持有线麦克风阵列或无线蓝牙音频设备。
...
1. **有线模式**: 默认使用 Wheeltec 专用的麦克风阵列硬件。
2. **无线模式 (蓝牙)**: 
   - **注意**: 本机器人默认**没有**内置蓝牙板卡。
   - 如果您希望使用蓝牙音频设备（如蓝牙麦克风、蓝牙耳机），您需要**额外购买并安装**蓝牙板卡/模块（例如 USB 蓝牙适配器）。
   - **免责声明**: 蓝牙连接模式**未经过我们的闭环认证**。虽然软件层面提供了兼容性支持，但由于蓝牙硬件规格各异且存在信号干扰风险，我们不保证其在所有环境下的稳定性与识别准确率。
   - 硬件安装并配对成功后，方可使用蓝牙功能。
